...
model_chosen <- workflow() %>%
  add_recipe(recipe(primary_06 ~ sex + treatment + age,
                    data = week_11_train) %>%
              step_dummy(all_nominal())) %>%
  add_model(linear_reg() %>%
              set_engine("stan"))

model_chosen_2 <- workflow() %>%  
  add_recipe(recipe(primary_06 ~ solo + treatment,
                    data = week_11_train) %>%
  step_dummy(all_nominal()) %>%
  step_interact(~ starts_with("solo"):starts_with("treatment"))) %>%
  add_model(linear_reg() %>%
              set_engine("lm"))
...
metrics_1 <- model_chosen %>%
  fit_resamples(week_11_folds) %>%
  collect_metrics()

metrics_2 <- model_chosen_2 %>%
  fit_resamples(week_11_folds) %>%
  collect_metrics()
